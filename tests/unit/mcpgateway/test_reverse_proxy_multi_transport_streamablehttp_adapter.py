# -*- coding: utf-8 -*-
"""Location: ./tests/unit/mcpgateway/test_reverse_proxy_multi_transport_streamablehttp_adapter.py
Copyright 2025
...
# First-Party
from mcpgateway.reverse_proxy_multi_transport.transports.streamablehttp_adapter import SessionExpiredError, StreamableHttpAdapter
...
async def test_start_creates_http_client_sets_endpoint_and_receive_task(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.streamablehttp_adapter.httpx.AsyncClient", async_client_mock)
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.streamablehttp_adapter.asyncio.create_task", capture_task)
...
async def test_start_configures_https_ssl_context_with_custom_cert(monkeypatch) -> None:
    ...
    fake_ssl_context.check_hostname = False
    fake_ssl_context.verify_mode = ssl.CERT_NONE
    receive_task = asyncio.create_task(asyncio.sleep(10))
    async_client_mock = Mock()
    ssl_factory = Mock(return_value=fake_ssl_context)

    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.streamablehttp_adapter.ssl.create_default_context", ssl_factory)
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.streamablehttp_adapter.httpx.AsyncClient", async_client_mock)
    monkeypatch.setattr(
        "mcpgateway.reverse_proxy_multi_transport.transports.streamablehttp_adapter.asyncio.create_task",
        Mock(return_value=receive_task),
    )

    adapter = StreamableHttpAdapter("https://secure.example/mcp", cert="CERTDATA")

    await adapter.start()

    ssl_factory.assert_called_once_with(cadata="CERTDATA")
...
async def test_receive_stream_sleeps_until_cancelled(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.streamablehttp_adapter.asyncio.sleep", fake_sleep)
