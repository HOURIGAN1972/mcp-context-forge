# -*- coding: utf-8 -*-
"""Location: ./tests/unit/mcpgateway/test_reverse_proxy_multi_transport_websocket_adapter.py
Copyright 2025
...
import mcpgateway.reverse_proxy_multi_transport.transports.websocket_adapter as websocket_adapter_mod
...
async def test_connect_uses_secure_defaults_and_custom_cert(monkeypatch) -> None:
    ...
    monkeypatch.setattr(websocket_adapter_mod, "websockets", Mock(connect=connect_mock))
    monkeypatch.setattr(websocket_adapter_mod.ssl, "create_default_context", ssl_factory)
    monkeypatch.setattr(websocket_adapter_mod.asyncio, "create_task", Mock(return_value=receive_task))

    adapter = WebSocketAdapter("gateway.example/base", "session-1", cert="CERTDATA")
