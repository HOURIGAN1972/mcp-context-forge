# -*- coding: utf-8 -*-
"""Location: ./tests/unit/mcpgateway/test_reverse_proxy_multi_transport_stdio_adapter.py
Copyright 2025
...
async def test_start_launches_subprocess_and_starts_stdout_reader(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.stdio_adapter.asyncio.create_subprocess_exec", create_subprocess_mock)
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.stdio_adapter.asyncio.create_task", create_task_mock)
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.stdio_adapter.asyncio.sleep", sleep_mock)
...
async def test_start_raises_runtime_error_when_process_exits_immediately(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "mcpgateway.reverse_proxy_multi_transport.transports.stdio_adapter.asyncio.create_subprocess_exec",
        AsyncMock(return_value=process),
    )
    stdout_task = asyncio.create_task(asyncio.sleep(10))
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.stdio_adapter.asyncio.create_task", Mock(return_value=stdout_task))
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.stdio_adapter.asyncio.sleep", AsyncMock())
...
async def test_stop_cancels_reader_and_terminates_running_process(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.stdio_adapter.asyncio.wait_for", wait_for_mock)
...
async def test_stop_force_kills_process_after_timeout(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.stdio_adapter.asyncio.wait_for", fake_wait_for)
