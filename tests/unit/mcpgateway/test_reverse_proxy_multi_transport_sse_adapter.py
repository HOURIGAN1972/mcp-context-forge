# -*- coding: utf-8 -*-
"""Location: ./tests/unit/mcpgateway/test_reverse_proxy_multi_transport_sse_adapter.py
Copyright 2025
...
async def test_start_creates_http_client_and_receive_task_for_http(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.sse_adapter.httpx.AsyncClient", FakeClient)
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.sse_adapter.asyncio.create_task", capture_task)
...
async def test_start_configures_https_ssl_context_with_custom_cert(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.sse_adapter.ssl.create_default_context", ssl_factory)
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.sse_adapter.httpx.AsyncClient", async_client_mock)
    monkeypatch.setattr("mcpgateway.reverse_proxy_multi_transport.transports.sse_adapter.asyncio.create_task", create_task_mock)

    adapter = SseAdapter("https://secure.example/sse", cert="CERTDATA")
