...
class SessionAffinity:
    ...
    async def forward_request_to_owner(
        ...
        try:
            ...
            if not owner:
                logger.info(f"[AFFINITY] Worker {get_worker_id()} | Session {mcp_session_id[:8]}... | Method: {method} | No owner → execute locally (new session)")
                return None  # No owner registered - execute locally (new session)

            owner_id = owner.decode() if isinstance(owner, bytes) else owner
            if owner_id == get_worker_id():
                logger.info(f"[AFFINITY] Worker {get_worker_id()} | Session {mcp_session_id[:8]}... | Method: {method} | We own it → execute locally")
            ...
            logger.info(f"[AFFINITY] Worker {get_worker_id()} | Session {mcp_session_id[:8]}... | Method: {method} | Owner: {owner_id} → forwarding")
